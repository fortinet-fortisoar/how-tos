...
class Usage:
    ...
    def __create_usage_file(self):
        usage_file_path = os.path.join(self.widget_docs_path, USAGE_FILE_NAME)
        return open(usage_file_path, 'w')

    def __create_usage_data(self, usage_file):
        usage_file.write(usage.substitute(
            readme_file=README_FILE_NAME, setup_file=SETUP_FILE_NAME))
        usage_file.close()
        logging.debug(
